...
async def get_current_comment(
    comment_uid: uuid.UUID, comments_uc: Annotated[CommentsUseCase, Depends(get_comments_uc)]
) -> DomainComment:
    try:
        return await comments_uc.get_current_comment(comment_uid=comment_uid)
